...
class VooiDC:
    ...
    def trade(self, proxy=None, pair_ticker="BTCUSD", amount="100", leverage=50):
        try:
            ...
            pair_mapping = {"BTCUSD": 1, "ETHUSD": 2, "TONUSD": 3}
            pair_ticker = random.choice(list(pair_mapping.keys())) 
            pair_id = pair_mapping.get(pair_ticker)

            if not pair_id:
                log(mrh + f"Invalid ticker provided: {pair_ticker}", flush=True)
                return {"success": False, "error": mrh + f"Invalid ticker provided"}

            trade_type = random.choice(['long', 'short'])
            url_create_position = f"{self.base_url}/trades/create"
            payload = {"type": trade_type, "pairId": pair_id, "amount": amount, "leverage": leverage}
            try:
                response_create = session.post(url_create_position, json=payload, headers=self.get_headers())
                if response_create.status_code == 201:
                    created_position = response_create.json()
                    created_pair_name = [key for key, value in pair_mapping.items() if value == created_position['pairId']][0]  # Get pair name by pairId

                    log(hju + f"Position {pth}{created_pair_name} {hju}created successfully", flush=True)
                    log(hju + f"Opening {pth}{created_position['type']} {hju}order at price: {pth}{created_position['openRate']}", flush=True)
                    log(hju + f"Open Date: {pth}{created_position['openDate']}", flush=True)
                    return {"success": True, "data": created_position}
                else:
                    log(mrh + f"Failed to create position, status code: {response_create.status_code}", flush=True)
                    return {"success": False, "error": mrh + f"Failed to create position, status code: {response_create.status_code}"}

            except Exception as e:
                log(mrh + f"Error creating position: {str(e)}", flush=True)
                return {"success": False, "error": str(e)}

        except Exception as e:
            log(f"Error in trading process: {str(e)}", flush=True)
            return {"success": False, "error": str(e)}
    ...
    def main(self):
        ...
        log(bru + f"Number of accounts: {pth}{len(data)}")
        log(bru + f"Total Proxies: {pth}{len(self.proxies)}" if self.proxies else bru + f"Total Proxies: {pth}No proxies used")
        log_line()
        ...
        while True:
            for i, init_data in enumerate(data):
                try:
                    proxy = None
                    log(hju + f"Account: {pth}{i + 1}/{len(data)}")
                    ...
                    login = self.login_new_api(init_data, proxy)
                    if login['success']:
                        log(hju + f"Username : {pth}{login['data']['name']}")
                        log(hju + f"vUSD Balance: {pth}{login['data']['balances']['virt_money']}")
                        log(hju + f"VT Balance: {pth}{login['data']['balances']['virt_points']}")
                        log(hju + f"Current invite: {pth}{login['data']['frens']['count']}/{login['data']['frens']['max']}")
                        self.autotrade(proxy)
                        if self.open_position:
                            self.trade(proxy)
                        else:
                            log(bru + f"Auto open position is not activated")

                        if self.auto_complete_task:
                            self.complete_quest(proxy)
                        else:
                            log(bru + f"Auto complete quest is not activated")

                        if self.auto_play_game:
                            self.play_tapping_game(proxy)
                        else:
                            log(bru + f"Auto play tapping game is not activated")

                        log(htm + "~" * 38)
                        countdown_timer(self.account_delay)

                    else:
                        log(mrh + f"Login failed! {login['error']}")

                except Exception as e:
                    log(f"Something wrong with account {i + 1} check last.log")
                    log(f"{str(e)}")
